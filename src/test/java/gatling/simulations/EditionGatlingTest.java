 ...
 */
public class EditionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all editions").get("/api/editions").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new edition")
                        .post("/api/editions")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"dayOfPublication\": 0" + ", \"monthOfPublication\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_edition_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created edition").get("${new_edition_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created edition").delete("${new_edition_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Edition entity").exec(scn);
