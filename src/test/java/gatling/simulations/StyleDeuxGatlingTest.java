...
/**
 * Performance test for the StyleDeux entity.
 */
public class StyleDeuxGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all styleDeuxes").get("/api/style-deuxes").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new styleDeux")
                        .post("/api/style-deuxes")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"styleOfTextDeux\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_styleDeux_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created styleDeux").get("${new_styleDeux_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created styleDeux").delete("${new_styleDeux_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the StyleDeux entity").exec(scn);
